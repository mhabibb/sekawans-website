...
<section class="container py-5 col-lg-10">
  <div class="article-header d-flex flex-column align-items-center mb-2">
    <a href="{{ route('documents.index') }}" class="btn text-muted mb-3">
      <i class="fa-solid fa-arrow-left"></i> Kembali</a>
    <div class="d-flex mb-3">
      <a href="{{ route('documents.edit', $document->id) }}" class="btn btn-warning btn-sm mx-2">
  ...
  function deleteDocument(id) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
            type: "DELETE",
            url: "{{ route('documents.destroy', $document->id) }}",
            data: id,
        })
